...
/-! Internal packed-bit helpers. Consumers use `import all`; these declarations
are not exported through the public term API. -/
...
def bitSize (bytes : ByteArray) (lastBits : Fin 8) : Nat :=
...
def toBits (bytes : ByteArray) (lastBits : Fin 8) : List Bool :=
