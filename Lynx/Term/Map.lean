...
private def findEntry (xs : Entries) (q : Term) : Option (Term × Term) :=
  xs.find? fun e => decide (exactCompare q e.1 = .eq)
...
@[simp] private theorem findEntry_cons (k v : Term) (xs : Entries) (q : Term) :
    findEntry ((k,v) :: xs) q =
      if exactCompare q k = .eq then some (k,v) else findEntry xs q := by
  by_cases h : exactCompare q k = .eq <;> simp [findEntry, List.find?, h]
...
@[simp] theorem find_put (xs : Entries) (k v q : Term) :
    find q (put k v xs) =
      if exactCompare q k = .eq then some v else find q xs := by
  by_cases h : exactCompare q k = .eq <;> simp [find, h]
...
theorem compare_eq_of_find (a b : Entries)
    (h : ∀ q, Option.Rel (fun a b => compare a b = .eq) (find q a) (find q b)) :
    compare (.map a) (.map b) = .eq := by
  exact map_compare_eq_of_lookup a b h
